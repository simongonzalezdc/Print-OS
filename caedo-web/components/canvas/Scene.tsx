...
function SceneContent() {
  ...
  // Pre-generate build volume geometry to prevent memory leaks from re-allocation
  const buildVolumeGeometry = useMemo(() => {
    const geo = new THREE.BoxGeometry(SCENE.BUILD_PLATE.X, SCENE.BUILD_PLATE.Z, SCENE.BUILD_PLATE.Y);
    const edges = new THREE.EdgesGeometry(geo);
    // Note: geo.dispose() will be called when edges is disposed if it was just a helper,
    // but here we should dispose of both eventually.
    return { geo, edges };
  }, []);
  ...
  useEffect(() => {
    return () => {
      buildVolumeGeometry.geo.dispose();
      buildVolumeGeometry.edges.dispose();
    };
  }, [buildVolumeGeometry]);
  ...
  return (
    <>
      ...
      <lineSegments 
        position={[0, SCENE.BUILD_PLATE.Z / 2, 0]} 
        geometry={buildVolumeGeometry.edges}
      >
