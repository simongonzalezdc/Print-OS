...
import { Download, Loader2, FileBox, Info, Send } from 'lucide-react';
import { cn } from '@/lib/utils';
import { useSceneStore } from '@/lib/scene/store';
import { ExportFormat } from '@/types';
import { sceneObjectToMesh3MF } from '@/lib/export/3mf';
import { exportToSTLBinary, sceneObjectToSTLMesh } from '@/lib/export/stl';
...
export function ExportPanel() {
  ...
  useEffect(() => {
    ...
    workerRef.current = new Worker(new URL('@/lib/export/worker', import.meta.url));
  ...
  const defaultFilename = objectsToExport.length === 1
    ...
    : `Caedo_Export_${new Date().toISOString().split('T')[0]}`;
  ...
  const handleExport = async () => {
    ...
    if (!workerRef.current) {
      toast.error('Export worker not initialized');
      return;
    }
    ...
    try {
      ...
      const onWorkerMessage = (e: MessageEvent) => {
        ...
        if (e.data.type === 'EXPORT_SUCCESS') {
          const { blob, filename: resultFilename } = e.data.payload;
      ...
      if (exportFormat === '3mf') {
        ...
        if (meshes3MF.length === 0) {
          toast.error('No valid meshes to export');
          setIsExporting(false);
          return;
        }
        ...
        if (combinedMesh.indices.length === 0) {
          toast.error('No valid mesh to export');
          setIsExporting(false);
          return;
        }
        ...
        workerRef.current.postMessage({
          type: triangleCount > 10000 ? 'EXPORT_STL_BINARY' : 'EXPORT_STL_ASCII',
          id: exportId,
          payload: combinedMesh
        });
      }
    } catch (error) {
  ...
  const handleHandoff = async () => {
    if (objectsToExport.length === 0) {
      toast.error('No objects to handoff');
      return;
    }
    ...
    try {
      ...
      const combinedMesh = mergeExportMeshes(meshesSTL);
      const result = exportToSTLBinary(combinedMesh);

      if (result.success && result.blob) {
        // Calculate total grams for the handoff
        const totalVolume = objectsToExport.reduce((acc, obj) => acc + (obj.stats?.volume || 0), 0);
        const estimatedGrams = (totalVolume / 1000) * 1.25; // Default density assumption

        // Set state in store for the business page to pick up
        setPendingBusinessAnalysis({
          stlBlob: result.blob,
          objects: objectsToExport,
          totalGrams: estimatedGrams
        });

        toast.success('Preparing Business Case...', {
          description: 'Transferring design data to Market Intelligence module.'
            });
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6 scrollbar-thin">
        ...
        <div className="pt-4 space-y-3">
          ...
          <button
            ...
            disabled={isExporting || isHandoffRunning || !hasValidObjects}
            className={cn(
              'w-full flex items-center justify-center gap-2 py-3 px-4 rounded-lg font-medium transition-all border shadow-sm',
              hasValidObjects && !isExporting && !isHandoffRunning
                ? 'bg-secondary text-foreground hover:bg-secondary/70 border-border'
                : 'bg-secondary/20 text-muted-foreground border-transparent cursor-not-allowed'
            )}
          >
            {isHandoffRunning ? (
              <>
              ...
              <>
                ...
                <span>Handoff to Caedo API</span>
              </>
            )}
          </button>

          {/* Orca Slicer Deep Link */}
          <button
            onClick={() => {
              // Orca Slicer supports orcaslicer:// URLs to open files or web pages
              // For a local file, we would normally download first or use a local path
              // Here we'll simulate the "one-click" by suggesting the link
              toast.info('Opening Orca Slicer...', {
                description: 'Ensure Orca Slicer is installed and "Allow deep links" is enabled.'
              });
              window.location.href = `orcaslicer://open?file=${encodeURIComponent(window.location.origin + '/api/v1/printfarm/projects/last-export')}`;
            }}
            disabled={!hasValidObjects}
            className={cn(
              'w-full flex flex-col items-center justify-center py-3 px-4 rounded-lg font-medium transition-all border border-orange-500/30 bg-orange-500/5 hover:bg-orange-500/10 text-orange-400 group',
              !hasValidObjects && 'opacity-50 grayscale cursor-not-allowed'
            )}
          >
            <div className="flex items-center gap-2">
              <span className="w-4 h-4 bg-orange-500 rounded-full flex items-center justify-center text-[10px] text-black font-black">O</span>
              <span>Slice in Orca Slicer</span>
            </div>
            <span className="text-[9px] font-bold opacity-50 group-hover:opacity-100">DIRECT_LINK_INTEGRATION</span>
          </button>
