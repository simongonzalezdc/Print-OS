 ...
 */
export function VoiceInput({ onTranscript, className, disabled, size = 'md' }: VoiceInputProps) {
      ...
      try {
        ...
        recognition.onresult = (event: SpeechRecognitionEvent) => {
          ...
          if (result.isFinal) {
            ...
            // Check for wake word "caedo"
            const hasWakeWord = lowerText.includes('caedo');
            
            // Clean text by removing wake word variations
            ...
            if (hasWakeWord || isListening) {
              ...
              const wasCommand = processVoiceCommand(cleanText);
              
              // If not a system command, send to AI for generation
              if (!wasCommand && cleanText.length > 0) {
  ...
  const canToggle = isSupported && !error && !disabled;
