 ...
 */
export function preprocessJSCADCode(code: string): string {
  let processed = code;
 ...
 * 
 * Supports Level of Detail (LOD) via segment count reduction.
 */
export async function executeJSCAD(code: string, lod: 'preview' | 'draft' | 'final' = 'final'): Promise<JSCADGeometry> {
  // Validate and optimize the code before preprocessing
  const validation = validateJSCADCode(code);
  lastValidationResult = validation;
  
  // Inject LOD-specific segment counts if detected
  let lodCode = validation.optimizedCode;
  if (lod === 'preview') {
    lodCode = lodCode.replace(/segments:\s*\d+/g, 'segments: 8');
  } else if (lod === 'draft') {
    lodCode = lodCode.replace(/segments:\s*\d+/g, 'segments: 16');
  }
  
  // Preprocess code to make it compatible with worker environment
  const processedCode = preprocessJSCADCode(lodCode);
  
  // Check cache first for performance
  const cacheKey = `${processedCode.trim()}_${lod}`;
