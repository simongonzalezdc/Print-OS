...
import { SceneStore, SceneObject, Project, HistoryEntry, DEFAULT_SCENE_SETTINGS } from '@/types';
...
/**
 * Zustand store for Caedo 3D scene state management.
 *
  ...
  subscribeWithSelector(
    immer((set, get) => ({
      ...
      openProject: (_projectId: string) => {
        // This is a stub, actual loading would happen from storage
        return null;
      },
      ...
      redo: () => {
        ...
        set((state) => {
          applyHistoryToState(state, entry, false);
          state.historyIndex++;
        });
      },
      ...
      // Handoff State
      handoff: {
        pendingBusinessAnalysis: null,
        pendingDesignPrompt: null,
      },

      setPendingBusinessAnalysis: (data) => {
        set((state) => {
          state.handoff.pendingBusinessAnalysis = data;
        });
      },

      setPendingDesignPrompt: (prompt) => {
        set((state) => {
          state.handoff.pendingDesignPrompt = prompt;
        });
      },
    }))
  )
...
function addToHistory(
  ...
  // Strip large binary data from history to prevent memory bloat
  const stripMeshData = (objs: SceneObject[]) => objs.map(o => ({
    ...o,
    meshData: undefined,
    normals: undefined
  }));
  ...
  const entry = {
    ...
    objectIds,
    previousState: stripMeshData(previousState),
    newState: stripMeshData(newState),
  };
...
/**
 * Apply a history entry to the provided state (for undo/redo)
 */
function applyHistoryToState(state: SceneStore, entry: HistoryEntry, reverse: boolean) {
  ...
  const restoreObject = (obj: Partial<SceneObject>) => {
    if (obj.id) {
      ...
      if (existingObj) {
        state.objects.set(obj.id, { ...existingObj, ...obj } as SceneObject);
      } else {
