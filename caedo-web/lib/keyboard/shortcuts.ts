 ...
 * 
 * Handles global keyboard shortcuts for Caedo 3D editor
 */
...
export function setupKeyboardShortcuts() {
  const handleKeyDown = (event: KeyboardEvent) => {
    ...
    // Ctrl+E / Cmd+E: Export to 3MF
    if (ctrlOrCmd && event.key === 'e') {
      event.preventDefault();
      // This would trigger the export panel or direct export
      return;
    }

    // Ctrl+G / Cmd+G: Generate with AI
    if (ctrlOrCmd && event.key === 'g') {
      event.preventDefault();
      // Focus AI input or trigger generation
      return;
    }
...
export function getShortcutsList(): Array<{ keys: string; description: string }> {
  ...
  return [
    ...
    { keys: `${mod}+E`, description: 'Export to 3MF' },
    { keys: `${mod}+G`, description: 'Focus AI Command' },
    { keys: `${mod}+/`, description: 'Show Help' },
  ];
}
