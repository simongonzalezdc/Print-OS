...
// Rate limiting - using proper map updates
const rateLimitMap = new Map<string, { count: number; resetTime: number }>();

/**
 * Rate limiting check with proper map updates
 */
function checkRateLimit(clientId: string): boolean {
  ...
  // Update count and save back to map
  userLimit.count++;
...
export async function POST(request: NextRequest) {
  // #region agent log
  fetch('http://127.0.0.1:7242/ingest/d78ad385-ebb0-4248-824b-eb05554e81c5', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ location: 'route.ts:POST', message: 'Received generate request', data: {}, timestamp: Date.now(), sessionId: 'debug-session', runId: 'run1', hypothesisId: '2' }) }).catch(() => { });
  // #endregion
  ...
  try {
    ...
    // Create AI provider with proper typing
    let provider: LanguageModel;
    try {
      const providerInstance = createAIProvider();
      provider = providerInstance as LanguageModel;
      // #region agent log
      fetch('http://127.0.0.1:7242/ingest/d78ad385-ebb0-4248-824b-eb05554e81c5', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ location: 'route.ts:POST', message: 'Provider created', data: { providerName: 'configured' }, timestamp: Date.now(), sessionId: 'debug-session', runId: 'run1', hypothesisId: '2' }) }).catch(() => { });
      // #endregion
    } catch (error) {
    ...
    // Build system prompt with full context (printer profile, user preferences, scene)
    ...
    try {
      // #region agent log
      fetch('http://127.0.0.1:7242/ingest/d78ad385-ebb0-4248-824b-eb05554e81c5', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ location: 'route.ts:POST', message: 'Starting streamText', data: { userPrompt, hasImages: images.length > 0 }, timestamp: Date.now(), sessionId: 'debug-session', runId: 'run1', hypothesisId: '2' }) }).catch(() => { });
      // #endregion
      ...
      if (images.length > 0) {
        console.log('[AI Route] Processing', images.length, 'images for multi-modal request');
      ...
      const result = await Promise.race([
        streamText({
          ...
          onFinish: ({ text }) => {
            console.log('AI generation completed:', text.length, 'characters');
            // #region agent log
            fetch('http://127.0.0.1:7242/ingest/d78ad385-ebb0-4248-824b-eb05554e81c5', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ location: 'route.ts:streamText:onFinish', message: 'Stream completed', data: { length: text.length }, timestamp: Date.now(), sessionId: 'debug-session', runId: 'run1', hypothesisId: '4' }) }).catch(() => { });
            // #endregion
          },
          onError: ({ error }) => {
            console.error('AI streaming error:', error);
            // #region agent log
            fetch('http://127.0.0.1:7242/ingest/d78ad385-ebb0-4248-824b-eb05554e81c5', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ location: 'route.ts:streamText:onError', message: 'Stream error', data: { error: String(error) }, timestamp: Date.now(), sessionId: 'debug-session', runId: 'run1', hypothesisId: '2' }) }).catch(() => { });
            // #endregion
          },
        }),
        timeoutPromise
      ...
      // #region agent log
      fetch('http://127.0.0.1:7242/ingest/d78ad385-ebb0-4248-824b-eb05554e81c5', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ location: 'route.ts:catch', message: 'Caught error', data: { error: error instanceof Error ? error.message : String(error) }, timestamp: Date.now(), sessionId: 'debug-session', runId: 'run1', hypothesisId: '5' }) }).catch(() => { });
      // #endregion
      if (timeoutId) {
      ...
      if (error instanceof Error && error.message === 'Request timeout') {
        return NextResponse.json(
          { error: 'Request timeout. Please try a simpler prompt.' },
          { status: 408 }
        );
      }
