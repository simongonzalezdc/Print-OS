...
export function estimatePrintTime(
  ...
  // Very rough estimate: assume average travel per layer
  const avgLayerLength = Math.sqrt((stats.volume || 1) / Math.max(height, 1)) * 4; // perimeter estimate
  const totalLength = layers * avgLayerLength * (1 + infillPercent / 100);
